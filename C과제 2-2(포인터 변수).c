...
int main(void)
{
	char* ptr_sent; //포인터 변수 ptr_sent 선언 (전체 문자열)
	...
	ptr_sent = sent; //전역 포인터 변수 ptr_sent에 전체 문자열의 시작 주소값을 저장한다.
	...
	*/
	do
	{
		first = input(ptr_sent); //input 함수에 ptr_sent의 값을 전달한다.
	} while (first != '#');
...
char input(char* ptr_sent)
{
	...
	char* ptr_input; //포인터 변수 ptr_input 선언 (부분 문자열)
	...
	printf("대소문자를 변환할 문자열을 입력하시오 : "); //화면에 '대소문자를 변환할 문자열을 입력하시오 : '를 출력한다.
	gets(input_chg); //문자열을 받아 부분 문자열의 배열에 저장한다.
	ptr_input = input_chg; //전역 포인터 변수 ptr_input에 부분 문자열의 시작 주소값을 저장한다.

	if (ptr_input[0] == '#') //부분 문자열의 첫 글자로 #이 입력된 경우, main 함수로 돌아가 프로그램을 종료한다.
	{
		return ptr_input[0];
	}
	...
	for (j = 0; j < 1000; j++) //for문을 사용해 전체 문자열에 부분 문자열의 첫 글자가 포함되는지 확인한다.
	{
		...
		if (ptr_sent[j] == ptr_input[0])
		{
			...
			{
				if (ptr_sent[j] == ptr_input[k]) //같다면, j와 r의 값에 1을 더해준다.
				{
					j++;
					r++;
				}
			...
			if (r == length) //j의 증가 정도인 r이 length와 같은 경우
			{
				...
				return ptr_input[0]; //부분 문자열을 첫 번째 글자를 반환하며 input 함수를 빠져나간다.
			}
	...
	{
		printf("결과 : %s을 찾을 수 없습니다.\n", ptr_input); //화면에 '결과 : %s을 찾을 수 없습니다.\n'를 출력한다. (s: ptr_input이 가리키는 문자열)
	}
